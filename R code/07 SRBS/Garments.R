...
garments <- function(time, stocks, auxs){
  with(as.list(c(stocks, auxs)),{
    ...
    Change_In_Discard_Rate <- (Desired_Fashion_Obsolescence_Rate-Fashion_Obsolescence_Rate) *
    ...
    return (list(c(d_GIC_dt,d_TD_dt,d_GDR_dt,d_EGD_dt), 
                 ...
                 A3_ST=Start_Time,
                 P1_DFOR=Desired_Fashion_Obsolescence_Rate,
...
run_scenario <- function(run_id=1,
                         ...
                         Init_Obsolescence_Rate=0.9,
                         Smoothing_Constant=0.5,
                         Start_Time=0,
                         Adjustment_Fraction=0.5,
                         Desired_Fashion_Obsolescence_Rate=0.5,
  ...
  auxs <- c(Smoothing_Constant=Smoothing_Constant,
            ...
            Desired_Fashion_Obsolescence_Rate=Desired_Fashion_Obsolescence_Rate,
  ...
  stocks <- c(Garments_in_Circulation=Init_Garments_in_Circulation,
              Total_Discards=0,
              Fashion_Obsolescence_Rate=Init_Obsolescence_Rate,
              Expected_Garments_Discards=Init_Garments_in_Circulation*
                            Init_Obsolescence_Rate)
  res <- ode(y=stocks, 
             times=simtime, 
             func = garments, 
             parms=auxs, 
